...
void displaySymbolTable(){
	printf("\nSymbol table\n<Sno , LexemeName, Type, Size, ReturnType>\n\n");
	for(int i=0;i<MAX_HASH_SIZE;i++){
		if(hashSymbol[i] == NULL) {
			...
			while(cur){
				printf("%d\t %s\t %s\t %s\t %d\t %s\t\n",cur->sno,cur->lexemeName,cur->tokenName, cur->type, cur->size, cur->returntype);
				cur = cur->next;
			}
		}
	}
...
void insertSymbolTable(char lexemeName[100], char type[20], char returntype[20], char tokenName[100]){
